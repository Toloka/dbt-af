        ...
        for field_name in list(values):
            ...
            if field_name not in required_fields:
                ...
                if isinstance(extra[field_name], (list, tuple)):
                    # brackets [] are not supported for array values in most warehouses
                    # HACK: if it's empty collection, we pass empty string. It will work ONLY with string fields, but
                    # it's impossible to fetch a correct type of the field from a warehouse, and it's impossible to
